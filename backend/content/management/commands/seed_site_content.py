...
<li>сведения о просмотрах страниц и карточек товаров (путь URL, тип
страницы, дата) — для внутренней статистики работы сайта
(обязательные cookie, разд.&nbsp;8.1); без передачи сторонним
счётчикам;</li>
...
<li>внутренняя статистика просмотров страниц и товаров (обязательные
cookie, разд.&nbsp;8.1) — без сторонних счётчиков;</li>
<li>дополнительная статистика посещений через Яндекс.Метрику / GA4 —
<strong>только</strong> при отдельном согласии на аналитические cookie
(разд.&nbsp;8.2);</li>
...
ключ <code>hoocon-theme</code>);</li>
<li>внутренняя статистика просмотров: какие страницы и карточки товаров
открывали (путь, тип, агрегированные счётчики на стороне сайта).
Идентификация визита — по сессии Django; данные не передаются
Яндекс.Метрике, Google Analytics и иным сторонним счётчикам.
Нужны для работы сервиса (улучшение каталога и контента).</li>
</ul>
...
означают отказ от необязательных cookie: сторонние счётчики аналитики
не подключаются (или перестают подключаться при следующих загрузках
страницы). Внутренняя статистика просмотров (разд.&nbsp;8.1) при этом
продолжает работать. Рекламные cookie на сайте не используем.</p>
