...
def resolve_bracket_for_drive(drive_family: str) -> str:
    """Pick bracket SKU for a selected drive family.

    BR-ML is required only for DA…FU series; other families use BR-M.
...
def build_ball_valve_kit_options(sku: SKU) -> dict[str, Any] | None:
    ...
    Returns:
        ``None`` for non-valves or when no drives are known; otherwise dict with
        ``drive_families``, ``suffixes``, ``bracket_by_drive``, ``bracket_hint``.
    """
    ...
    if not families:
        return None
    bracket_by_drive = {family: resolve_bracket_for_drive(family) for family in families}
    return {
        "drive_families": families,
        "suffixes": list(_DRIVE_SUFFIXES),
        "bracket_by_drive": bracket_by_drive,
        "bracket_hint": format_bracket(tuple(families)),
    }
