...
class SiteSettings(models.Model):
    ...
    lead_routing_mode: models.CharField = models.CharField(
        ...
        help_text=(
            "Пул — активные сотрудники группы «Менеджер» с адресом в профиле "
            "(логин). Выкл: письмо на LEAD_NOTIFY_EMAIL, без назначения. "
            "«Назначать…»: равномерная очередь; при одном менеджере всегда "
            "он. Пустой пул = как Выкл."
        ),
    )
