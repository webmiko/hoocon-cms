...
- Первый публичный релиз (GA): канал beta снят, версия **1.0** в футере
  без префикса `v`.
- Главная: квиз подбора оборудования (`#podbor`) — приводы, шаровые краны,
  комплекты, адаптеры BR-M / BR-ML; карусель SKU и переход в каталог.
- Страница `/dokumentaciya`: поиск инструкций и паспортов, скачивание PDF
  и ZIP по семейству SKU.

### v0.1.30 beta — 2026-08-21

- Страница `/dokumentaciya`: поиск инструкций и паспортов, скачивание PDF
  и ZIP по семейству SKU (`GET /api/catalog/docs/`).

### v0.1.29 beta — 2026-08-19

- Карточки SKU: PDF паспорта изделия в «Документы» рядом с инструкцией
  (`_инструкции-pdf/паспорт изделия/`, `attach_manual_pdfs --series passport`).

### v0.1.28 beta — 2026-08-18

- Статьи: таблицы на узком экране складываются в карточки с подписями из шапки.

### v0.1.27 beta — 2026-08-18

- SPA: `/sw.js` без кэша; вкладка сама подтягивает новую сборку по `/api/health/`.

### v0.1.26 beta — 2026-08-18

- Склад: строка 1С «артикул 4–20 мА» пишет отдельный остаток; на сайте только
  «спецзаказ · на складе», без количества.
- Каталог/формы: контраст AA, autocomplete, honeypot скрыт, фильтры с 960px,
  spa_index `/rfq` и `/compare` без 404-title.
- Фото SKU снова с `/media` (без blob:); кроп схем DAFU из landscape PDF пака.

### v0.1.25 beta — 2026-08-11

- Новости: категории (Продукты / Статьи / Мероприятия / Компания), фильтр и
  сортировка на `/novosti`.
- Admin: на мобильных карточках changelist скрыты пустые поля.

### v0.1.24 beta — 2026-08-11

- Статья MU/MQU/HV (скорость хода, Q/UQ/QX), обложки; related SKU не
  путает Q с QX. Публикация по расписанию 17.08 с новостью и соцанонсом.

### v0.1.23 beta — 2026-08-11

- Go-live по расписанию: Celery beat раз в 15 мин создаёт новость о статье
  и анонсирует в соцсети (tipy / pitanie / MU-HV / analog-belimo).

### v0.1.22 beta — 2026-08-11

- Статьи: гайды подбора (момент, сертификаты CE/UL/EAC live; остальные по
  расписанию), новость `articles-podbor-i-sertifikaty`, локальный превью
  через `CONTENT_SHOW_SCHEDULED`.
...
- Admin: аварийный вход супер-админа при Email OTP — постоянный пароль
  (`?mode=password`) и одноразовые резервные коды (`?mode=recovery` /
  поле на `/admin/otp/`); генерация кодов в карточке пользователя.
- Новость про адаптеры BR-M / BR-ML в seed и миграции (обложка с BR-M).
